...
const Hover = ({ markerinfo }) => {
  ...
  return <div className="h-1/4 w-full bg-white absolute bottom-0 rounded-r-3xl rounded-t-3xl p-5 flex flex-col min-h-[200px] overflow-hidden">
    <p className="text-sm font-medium">{markerinfo.display_name}</p>
    <p className="text-sm font-normal italic">@{markerinfo.username}</p>
    <img src={markerinfo.image_url} className="w-full h-24 object-cover mt-2"/>
